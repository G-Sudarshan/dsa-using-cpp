...
Easy Accuracy: 53.25% Submissions: 17348 Points: 2
...
class Solution{
    ...
    long long min_diff = LLONG_MAX; // initialize min_diff to max
    long long diff;

    for(int i = 0 ; i+m-1<n; i++){
    	diff = a[i+m-1]-a[i]; // calculate diff
    	
    	if(diff < min_diff)
    	 min_diff = diff;  // modify min_diff if needed
    }
